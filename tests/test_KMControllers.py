...
class BasicTestSuite(unittest.TestCase):
    ...
    def test_float2bytes(self):
        assert KMControllers.float2bytearray(-3)==bytearray([0xC0,0x40,0x00,0x00])
    
    def test_bytes2float(self):
        float_value=-3.4
        assert abs(KMControllers.bytearray2float(bytearray([0xC0,0x59,0x99,0x99]))-float_value) < 0.0001
